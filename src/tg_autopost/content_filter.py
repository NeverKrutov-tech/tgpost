...
# ponytail: ad phrases shared with telegram_channel.py and telethon_channel.py
# to keep ingestion and publish-time checks in sync.
_AD_PHRASES = [
    "\u0447\u0438\u0442\u0430\u0442\u044C \u043F\u0440\u043E\u0434\u043E\u043B\u0436\u0435\u043D\u0438\u0435",
    "\u0447\u0438\u0442\u0430\u0439\u0442\u0435 \u043F\u0440\u043E\u0434\u043E\u043B\u0436\u0435\u043D\u0438\u0435",
    "\u043F\u0440\u043E\u0434\u043E\u043B\u0436\u0435\u043D\u0438\u0435 \u0432",
...
def is_ad(text: str) -> bool:
    """True if text contains known ad/funnel phrases (case-insensitive)."""
    low = text.lower()
    return any(p in low for p in _AD_PHRASES)


def is_flagged(text: str) -> bool:
    return is_political(text) or is_adult(text) or is_ad(text)
